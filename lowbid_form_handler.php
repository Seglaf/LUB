...
//TO REFRESH ALL UBs INCASE OF EDIT=======================================
			...
			$allUBs->execute();
			$allUBs = $allUBs->fetchAll();
//=========================================================================	
			echo json_encode(array('renderHtml' => $this->html, 'HUBs' => $this->ID_HUBs, 'NUBs' => $this->ID_NUBs, 'LUB' => $this->ID_LUB, 'oldLUB' => $this->old_LUB, 'foundLUB' => $this->found_LUB, 'bidTokens' => $this->bid_tokens, 'notEnuffTokens' => NULL, 'test' => $this->test, 'progress' => $this->progress));
		}else{
...
//==========================================^^^^^^^^^^MAKE SEPERATE FUNCTION ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^===================================					
				...
				//CHECK IF LUB
				$selected_bids = $this->db->prepare("SELECT COUNT(*) FROM gameplay WHERE product_ID=$this->selected_product_ID AND bid_price < $user_bid;");

				$selected_bids->execute();
				$selected_bids = $selected_bids->fetchColumn();
...
//FIX!!!! SEPERATE FUNCTION====================================================	
		if($user_bid_count == $bid_count){
			...
			$query = $this->db->prepare("UPDATE product SET winner='$this->username'");
			$query->execute();
		}
	...
	private function check_cap($total_user_bids, $bid_cap){

		if($total_user_bids > $bid_cap){

			echo $this->html .= "<p>YOU MADE TOO MANY BIDS!</p>";
			exit();
		}elseif($total_user_bids == $bid_cap){
...
//========================================================================
	...
	private function query_user_value($column){
		...
		$query = $this->db->prepare("SELECT $column FROM user WHERE user_ID='$this->user_ID';");
		$query->execute();
		return $query->fetchColumn();
	}
...
/*DOUBLE CHECK */ 	$this->ID_LUB = "box_" . (floatval($this->LUB) / $this->range_min);
	...
	$this->html .= "<div class='missedLUB " . $animation_class . "'>YOU MISSED THE LOWEST UNIQUE BID</div>";
		...
		foreach($this->array_HUB as $HUB){
			...
			array_push($this->ID_HUBs, "box_" . (floatval($HUB) / $this->range_min));
			$this->html .= "<div class='annouceHUB " . $animation_class . "'>YOU OWN A HIGHER UNIQUE VALUE: $HUB</div>";
		}
